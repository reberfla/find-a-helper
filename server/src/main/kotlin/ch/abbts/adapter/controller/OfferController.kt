...
fun Application.offerRoutes(offerInteractor: OfferInteractor, userInteractor: UserInteractor) {
  routing {
    route("/v1/offers") {
      post {
        try {
      ...
      delete("/{id}") {
        try {
          ...
          call.respond(HttpStatusCode.OK, mapOf("message" to "Offer deleted"))
        } catch (e: WebserverError) {
          call.respond(e.getStatus(), e.getMessage())
        }
      }
