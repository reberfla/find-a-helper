    ...
    routing {
        route("/v1") {
            @KtorResponds(
                ...
                description = """ This api verifies a users credential or token from google and returns
                ...
                try {
                    ...
                    val verifiedUser = when (user.authenticationProvider) {
                        ...
                        AuthProvider.LOCAL -> {
                            user.password?.let { userInteractor.verifyLocalUser(user.email, it) }
                    ...
                    if (verifiedUser.id != null) {
                        ...
                        userInteractor.updateIssuedTime(verifiedUser.email, token.body.iat)
            ...
            authenticate("jwt-auth") {
                ...
                get("/{token}") {
                    try {
                        ...
                        call.respond(
                            HttpStatusCode.OK, UserDto.toDTO(user)
                        )
                    } catch (e: WebserverError) {
