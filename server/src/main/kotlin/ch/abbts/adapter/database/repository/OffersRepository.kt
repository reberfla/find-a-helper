...
class OffersRepository {
    ...
    fun createOffer(offer: OfferModel): OfferModel {
        try {
            ...
            return transaction {
                ...
                val existing =
                    OffersTable.select {
                            (OffersTable.userId eq offer.userId) and
                                (OffersTable.taskId eq offer.taskId)
                        }
                        .count()
                ...
                val inserted =
                    OffersTable.insert {
                        ...
                        .resultedValues
                        ?.firstOrNull() ?: throw OfferCreationFailed()
    ...
    fun deleteOffer(userId: Int, offerId: Int): Boolean {
        return transaction {
            val offer =
                OffersTable.select {
                        (OffersTable.id eq offerId) and
                            (OffersTable.userId eq userId)
                    }
    ...
    fun hasUserSubmittedOfferForTask(userId: Int, taskId: Int): Boolean {
        return transaction {
            OffersTable.select {
                    (OffersTable.userId eq userId) and
