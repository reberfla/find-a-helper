...
class AuthenticationTest() {
    ...
    fun testUserAuthentication() = testApplication {
        application {
            val mockUserInteractor = mock<UserInteractor> {
                given(mock.verifyLocalUser(eq("mailb@example.ch"), any())).willAnswer { throw UserNotFound() }
                doNothing().whenever(mock).verifyLocalUser(eq("mail@example.ch"), any())
                doNothing().whenever(mock).updateIssuedTime(eq("mail@example.ch"), any())
            }
        ...
        val response = customClient.post("/v1/auth") {
        ...
        val responseNotExisting = customClient.post("/v1/auth") {
            contentType(ContentType.Application.Json)
            setBody(userNotExisting)
        }
        assertEquals(HttpStatusCode.NotFound, responseNotExisting.status)
        assertEquals(
            """{"message":"user not found"}""".trimIndent(), responseNotExisting.bodyAsText()
        )
    }
}
